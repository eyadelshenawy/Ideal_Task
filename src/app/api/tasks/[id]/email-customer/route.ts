...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const recipients = task.contactAssignees.filter((c) => c.email).map((c) => c.email as string);
  if (recipients.length === 0) {
    return NextResponse.json({ error: "This task has no assigned contact with an email — add one via Manage Contacts" }, { status: 400 });
  }
